...
CallbackReturn CMTopicSystem::on_init(
  const hardware_interface::HardwareComponentInterfaceParams & params)
{
  ...
  pal_names_subscriber_ =
    get_node()->create_subscription<pal_statistics_msgs::msg::StatisticsNames>(
    "~/names", rclcpp::SensorDataQoS(),
    [this](const pal_statistics_msgs::msg::StatisticsNames::SharedPtr pal_names) {
      pal_statistics_names_per_topic_[pal_names->names_version] = std::move(pal_names->names);
    });
  pal_values_subscriber_ =
    get_node()->create_subscription<pal_statistics_msgs::msg::StatisticsValues>(
...
hardware_interface::return_type CMTopicSystem::read(
  ...
  const rclcpp::Duration & /*period*/)
{
  ...
  if (it != end_it) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      if (names[i].rfind(prefix, 0) == 0) { // starts with prefix
...
hardware_interface::return_type CMTopicSystem::write(
  const rclcpp::Time & /*time*/,
  const rclcpp::Duration & /*period*/)
{
  // nothing to do here
...
PLUGINLIB_EXPORT_CLASS(
  controller_manager_topic_hardware_component::CMTopicSystem,
